...
class BooleanOperator(object):
    ...
    def __init__(
        self, precedence, eval_func, default_symbol_str, default_plain_english_str
    ):
...
_PRECEDENCE = {"ZERO": 0, "LOW": 1, "MEDIUM": 2, "HIGH": 3}


TT_NOT_OP = BooleanOperator(_PRECEDENCE["HIGH"], lambda a: not a, "~", "not")
...
TT_IMPL_OP = BooleanOperator(
    _PRECEDENCE["MEDIUM"], lambda a, b: (not a) or b, "->", "impl"
)
...
TT_XOR_OP = BooleanOperator(_PRECEDENCE["MEDIUM"], lambda a, b: a != b, None, "xor")
...
TT_XNOR_OP = BooleanOperator(_PRECEDENCE["MEDIUM"], lambda a, b: a == b, None, "xnor")
...
TT_AND_OP = BooleanOperator(_PRECEDENCE["LOW"], lambda a, b: a and b, "/\\", "and")
...
TT_NAND_OP = BooleanOperator(
    _PRECEDENCE["LOW"], lambda a, b: not (a and b), None, "nand"
)
...
TT_OR_OP = BooleanOperator(_PRECEDENCE["ZERO"], lambda a, b: a or b, "\\/", "or")
...
TT_NOR_OP = BooleanOperator(_PRECEDENCE["ZERO"], lambda a, b: not (a or b), None, "nor")
