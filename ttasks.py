...
def _print_sys_info():
    ...
    py_runtime = " ".join((platform.python_implementation(), platform.python_version()))
...
def test():
    ...
    suite = unittest.defaultTestLoader.discover(
        TESTS_DIR, pattern="test_*.py", top_level_dir=HERE
    )
    ...
    for file in doctest_files:
        suite.addTests(
            doctest.DocFileSuite(file, module_relative=False, **common_doctest_kwargs)
        )
...
TASKS = {"build-docs": build_docs, "serve-docs": serve_docs, "test": test}
