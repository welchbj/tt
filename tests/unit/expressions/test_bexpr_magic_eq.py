...
class TestExpressionMagicEquals(unittest.TestCase):
    def assert_equal_expressions(self, one, two):
        ...
        self.assertEqual(BooleanExpression(one), BooleanExpression(two))

    def assert_not_equal_expressions(self, one, two):
        """Assert two expressions (from strings) are equivalent."""
        self.assertNotEqual(BooleanExpression(one), BooleanExpression(two))
    ...
    def test_nested_binary_operator_expressions(self):
        ...
        self.assert_equal_expressions(
            "(A xor (B or C)) nand (D or E or F)", "(A xor (B or C)) nand (D or E or F)"
        )
        self.assert_equal_expressions(
            "(A xor (B or C)) nand (D or E or F)", "(A xor (B | C)) nand (D \\/ E || F)"
        )

        self.assert_not_equal_expressions(
            "(A xor (B or C)) nand (D or E or F)", "(A xor (B or C)) nand (D or G or F)"
        )
        self.assert_not_equal_expressions(
            "(A xor (B or C)) nand (D or E or F)", "(B or C) nand (D or G or F)"
        )
