...
class TestNodeToPrimitives(ExpressionTreeAndNodeTestCase):
    ...
    def test_no_change_to_binary_operators(self):
        ...
        for op in BINARY_OPERATORS:
            ...
            self.assertEqual(
                str(coalesced),
                "\n".join((op.default_plain_english_str, "`----A", "`----B")),
            )
    ...
    def test_constant_operands(self):
        ...
        coalesced = root.coalesce_negations()
        self.assertTrue(coalesced is not root)
        self.assertEqual(str(coalesced), "1")

        root = self.get_tree_root_from_expr_str("~1")
        coalesced = root.coalesce_negations()
        self.assertTrue(coalesced is not root)
        self.assertEqual(str(coalesced), "0")
    ...
    def test_single_negation(self):
        ...
        self.assertTrue(coalesced is not root)
        self.assertTrue(coalesced.l_child is not root.l_child)
        self.assertEqual(str(coalesced), "\n".join(("~", "`----A")))
    ...
    def test_multiple_negations(self):
        ...
        coalesced = root.coalesce_negations()
        self.assertTrue(
            coalesced.l_child is not root.l_child.l_child.l_child.l_child.l_child
        )
        self.assertEqual(str(coalesced), "\n".join(("~", "`----A")))
