...
class TestNodeMagicEq(ExpressionTreeAndNodeTestCase):
    def assert_equal_trees(self, one, two):
        ...
        self.assertEqual(
            self.get_tree_root_from_expr_str(one), self.get_tree_root_from_expr_str(two)
        )
    ...
    def test_chained_operator_expressions(self):
        ...
        self.assert_equal_trees("A or B or C or D", "A or B or C or D")
        self.assert_equal_trees(
            "(A or B or C) xor (D or E) xor (F and G and H and I)",
            "(A or B or C) xor (D or E) xor (F and G and H and I)",
        )
        self.assert_equal_trees(
            "~(A or B or C) and (D iff E)", "~(A or B or C) and (D iff E)"
        )

        self.assert_not_equal_trees(
            "(A or B or C) and D and (E or F)", "(A or B or C) and D and E"
        )
        self.assert_not_equal_trees(
            "A and (B nand C nand D)", "A and (B nand ~C nand D)"
        )

    def test_nested_binary_operator_expressions(self):
        """Test equality for expressions containing many levels."""
        self.assert_equal_trees("(A and B) or (C and D)", "(A and B) or (C and D)")
        self.assert_equal_trees("(A and B) or (C and D)", "(A and B) || (C & D)")
        self.assert_equal_trees(
        ...
        self.assert_not_equal_trees("(A and B) or (C and D)", "(A and B) or (D and C)")
        self.assert_not_equal_trees("(A and B) or (C and D)", "(B and A) or (C and D)")
        self.assert_not_equal_trees(
