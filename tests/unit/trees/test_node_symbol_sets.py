...
class TestNodeSymbolSets(ExpressionTreeAndNodeTestCase):
    def assert_symbol_sets(
        self, tree_root, expected_non_negated_symbol_str, expected_negated_symbol_str
    ):
        self.assertEqual(
            expected_non_negated_symbol_str, tree_root.non_negated_symbol_set
        )
        self.assertEqual(expected_negated_symbol_str, tree_root.negated_symbol_set)
    ...
    def test_operand_in_both_sets(self):
        ...
        root = self.get_tree_root_from_expr_str("(A or B) and (~A or C or ~B) and ~C")
