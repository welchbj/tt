...
class TestCoalesceNegations(unittest.TestCase):
    ...
    def test_from_boolean_expression_object(self):
        ...
        self.assert_coalesce_negations_transformation(BooleanExpression("~~A"), "A")

    def test_only_constant_expressions(self):
        """Test the transformation on negated expression objects."""
        self.assert_coalesce_negations_transformation("1", "1")
        self.assert_coalesce_negations_transformation("0", "0")
        self.assert_coalesce_negations_transformation("~0", "1")
        self.assert_coalesce_negations_transformation("~1", "0")
        self.assert_coalesce_negations_transformation("~~1", "1")
        self.assert_coalesce_negations_transformation("~~0", "0")
        self.assert_coalesce_negations_transformation("!!!1", "0")
        self.assert_coalesce_negations_transformation("not not not not not 0", "1")
        self.assert_coalesce_negations_transformation("~1 and ~0", "0 and 1")
        self.assert_coalesce_negations_transformation("~~~1 -> 0", "0 -> 0")
    ...
    def test_negated_single_operand(self):
        ...
        self.assert_coalesce_negations_transformation("A", "A")
    ...
    def test_negated_expression(self):
        ...
        self.assert_coalesce_negations_transformation("~(A or B)", "~(A or B)")

        self.assert_coalesce_negations_transformation("~~(A && B)", "A && B")

        # ensure that the closest negation to the expression is preserved
        self.assert_coalesce_negations_transformation("~~ not(A -> B)", "not (A -> B)")

        self.assert_coalesce_negations_transformation(
            "~ not ~ not !!(A iff B)", "A iff B"
        )
