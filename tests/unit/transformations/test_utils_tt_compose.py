...
class TestUtilsTtCompose(unittest.TestCase):
    ...
    def test_mixed_callables_and_transformations(self):
        ...
        f = tt_compose(
            self._custom_trans_func, _another_custom_trans_func, coalesce_negations
        )
        self.assertEqual(
            str(f),
            "_custom_trans_func -> _another_custom_trans_func -> coalesce_negations",
        )
    ...
    def test_modifiers_on_transformations(self):
        ...
        self.assertEqual(str(f(de_morgan_izable_expr)), "not (not not A or not not B)")

        f = tt_compose(apply_de_morgans, forever, coalesce_negations)
        self.assertEqual(str(f), "apply_de_morgans (inf times) -> coalesce_negations")
    ...
    def test_consecutive_modifiers(self):
        ...
        f = tt_compose(coalesce_negations, twice, repeat(3), repeat(4), forever)
