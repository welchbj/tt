...
class TestApplyDeMorgans(unittest.TestCase):
    ...
    def test_from_boolean_expression_object(self):
        ...
        self.assert_apply_de_morgans_transformation(
            BooleanExpression("not (A or B)"), "not A and not B"
        )

    def test_no_effect_expected(self):
        """Test scenarios where no change to the expression should occur."""
        self.assert_apply_de_morgans_transformation("A nand B", "A nand B")

        self.assert_apply_de_morgans_transformation("~A and B", "~A and B")

        self.assert_apply_de_morgans_transformation("A or B", "A or B")

    def test_negated_and(self):
        """Test the transformation on a negated AND."""
        self.assert_apply_de_morgans_transformation("~(A and B)", r"~A \/ ~B")

        self.assert_apply_de_morgans_transformation("(not (A and B))", "not A or not B")
    ...
    def test_compound_expression(self):
        ...
        self.assert_apply_de_morgans_transformation(
            "not (~(A or B) and ~(C and D))", r"not (~A /\ ~B) or not (~C \/ ~D)"
        )
    ...
    def test_multi_level_negated_expressions(self):
        ...
        self.assert_apply_de_morgans_transformation(
            "~(A and ~(B or ~(C and D) or D))", r"~A \/ ~(~B /\ ~(~C \/ ~D) /\ ~D)"
        )
