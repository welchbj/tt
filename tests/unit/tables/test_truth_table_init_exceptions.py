...
class TestTruthTableInitExceptions(TruthTableTestCase):
    def test_invalid_expr_type(self):
        ...
        self.helper_test_truth_table_raises(
            float(), expected_exc_type=InvalidArgumentTypeError
        )

    def test_only_constant_values(self):
        """Test an expression of only constant values."""
        self.helper_test_truth_table_raises(
            "0 nand (1 or 0)", expected_exc_type=NoEvaluationVariationError
        )

    def test_ordering_missing_symbols(self):
        """Test passing too few symbols in the symbol ordering."""
        self.helper_test_truth_table_raises(
            "A or B or C", ordering=["A", "C"], expected_exc_type=MissingSymbolError
        )
    ...
    def test_specify_both_expr_and_from_values(self):
        ...
        self.helper_test_truth_table_raises(
            "A or B", from_values="001x", expected_exc_type=ConflictingArgumentsError
        )

    def test_specify_neither_expr_or_from_values(self):
        """Test specifying neither the expression or starting values."""
        self.helper_test_truth_table_raises(
            None, from_values=None, expected_exc_type=RequiredArgumentError
        )
    ...
    def test_from_values_invalid_str_value(self):
        ...
        self.helper_test_truth_table_raises(
            None, from_values="00_0", expected_exc_type=InvalidBooleanValueError
        )
    ...
    def test_empty_str_from_values(self):
        ...
        self.helper_test_truth_table_raises(
            None, from_values="", expected_exc_type=InvalidArgumentValueError
        )

    def test_non_pow_2_from_values(self):
        """Test passing a non-power-of-2 number of input values."""
        self.helper_test_truth_table_raises(
            None, from_values="01x", expected_exc_type=InvalidArgumentValueError
        )
    ...
    def test_invalid_symbol_from_expression(self):
        ...
        self.helper_test_truth_table_raises(
            "A$ or B", expected_exc_type=InvalidIdentifierError
        )
    ...
    def test_invalid_symbols_in_ordering_from_expression(self):
        ...
        self.helper_test_truth_table_raises(
            "A or B", ordering=["A", "B", "B$"], expected_exc_type=ExtraSymbolError
        )

        self.helper_test_truth_table_raises(
            "A nand B", ordering=["A", "@B"], expected_exc_type=MissingSymbolError
        )
